...
MAKE_CHECK_URL  = os.environ["MAKE_CHECK_URL"]  # новый вебхук проверки дублей
...
- vat_rate: только явно указанная ставка НДС.
...
- zsk_color: Зелёный если "от зелёных"/"зелёный", Жёлтый, Красный — только если явно указано.
- company_type: только если явно написано "белый бизнес", "флагман" и т.д.
- white_business: только если явно указано.
...
def format_missing_msg(missing: list) -> str:
    ...
    lines.append("\nОтветь на все в одном сообщении или нажми кнопку.")
    return "\n".join(lines)
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "👋 *Привет!*\n\n"
        "Пересылай мне заявки из Telegram-групп.\n\n"
        "Я извлекаю только то что явно написано.\n"
        "Проверяю дубли по ИНН перед записью.\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await proc.edit_text(
            ...
            "Попробуй переслать ещё раз или напиши данные в формате:\n"
            "Компания: ...\nИНН: ...\nНазначение: ..."
        )
        return
...
async def _process_current(update: Update, session: dict, user_id: int):
    ...
    if inn:
        ...
        if dup.get("found"):
            ...
            dup_source = dup.get("source") or "неизвестный источник"
            dup_date = dup.get("updated_at") or "неизвестная дата"

            keyboard = InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("➕ Внести как новую", callback_data="dup_new"),
...
async def _save_current(update, session: dict, user_id: int):
    ...
    try:
        ...
        if ok:
            ...
            await reply.reply_text("⚠️ Ошибка записи в Make. Проверь сценарий.")
    except Exception as e:
    ...
    if session["current_idx"] < len(session["pending"]):
        ...
        if len(session["pending"]) > 1:
            r = getattr(update, 'message', None) or update.callback_query.message
            await r.reply_text(
...
async def handle_dup_new(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def handle_dup_update(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обновить старую запись — просто перезаписываем."""
    query = update.callback_query
    await query.answer()
    user_id = query.from_user.id
    session = sessions.get(user_id)
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(handle_dup_new,    pattern="^dup_new$"))
